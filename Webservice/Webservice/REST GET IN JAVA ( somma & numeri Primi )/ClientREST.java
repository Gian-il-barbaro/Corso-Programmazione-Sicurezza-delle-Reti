    ...
    public static void main(String[] args) {
        ...
        switch (args[0]) {
            case "calcola-somma": {
                ...
                System.out.printf("Tempo di risposta: %.3f ms\n", (t1 - t0) / 1e6);
                break;
            }
            case "numeri-primi": {
                ...
                if (result != null) {
                    ...
                    System.out.println("Somma = " + result.somma);
                    System.out.printf("Tempo di risposta: %.3f ms\n", (t1 - t0) / 1e6);
                }
                break;
            }
...
class PrimeResult {
    ...
    PrimeResult(int count, long somma, List<Integer> primes) {
        this.count = count;
        this.somma = somma;
        this.primes = primes;
    }
...
class RESTAPI {
    ...
    /**
     * Chiama /numeri-primi e restituisce sia il conteggio, la somma e la lista completa.
     */
    PrimeResult primeNumber(int a, int b) {
        ...
        // 2) Estrae la somma
        Pattern pSum = Pattern.compile("\"somma\"\\s*:\\s*(\\d+)");
        Matcher mSum = pSum.matcher(risposta);
        if (mSum.find()) {
            somma = Long.parseLong(mSum.group(1));
        }
        ...
        return new PrimeResult(count, somma, lista);
    }
