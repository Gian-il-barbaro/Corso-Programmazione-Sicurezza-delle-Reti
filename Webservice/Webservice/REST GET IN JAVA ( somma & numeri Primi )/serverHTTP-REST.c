    ...
    for(;;){
        ...
        if(strstr(url,"calcola-somma")){
            ...
            if(sscanf(url,"%*[^?]?param1=%d&param2=%d",&a,&b)!=2||a>b){
                ...
                /* Timer per misurare il tempo */
                struct timespec start, end;
                clock_gettime(CLOCK_REALTIME, &start);
                printf("Inizio il calcolo\n");

                /* Stima dimensione buffer */
                int estimated = (b - a + 1) * 6 + 128; 
                char *primiBuf = malloc(estimated);
                if (!primiBuf) {
                    send_json(connfd,500,"{ \"errore\": \"Memoria insufficiente\" }\n");
                    fclose(connfd);
                    continue;
                }

                /* Costruzione JSON array dei numeri primi */
                int counter = 0;
                long long somma = 0;   // somma dei numeri primi
                int offset = 0;

                for(int i=a;i<=b;++i){
                    if(is_prime(i)){
                        if(counter > 0) offset += snprintf(primiBuf+offset, estimated-offset, ",");
                        offset += snprintf(primiBuf+offset, estimated-offset, "%d", i);
                        somma += i;
                        ++counter;
                    }
                }
                ...
                /* Risultato finale */
                char *body = malloc(estimated + 256);
                if (!body) {
                    free(primiBuf);
                    send_json(connfd,500,"{ \"errore\": \"Memoria insufficiente\" }\n");
                    fclose(connfd);
                ...
                snprintf(body, estimated + 256,
                         "{ \"totale_numeri_primi\": %d, \"somma\": %lld, \"primi\": [%s] }\n",
                         counter, somma, primiBuf);
                ...
                printf("Calcolo completato in %.6f secondi\n", elapsed);
            }
        }
        else{
